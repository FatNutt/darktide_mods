...
local BuffBarSettings = mod:io_dofile('better_buff_management/scripts/mods/better_buff_management/hud/hud_element_buff_bar_settings')
...
local MAX_BUFFS = BuffBarSettings.max_buffs
...
local HudElementBuffBar = class('HudElementBuffBar', 'HudElementBase')
function HudElementBuffBar:init(parent, draw_layer, start_scale, definitions)
...
function HudElementBuffBar:event_player_buff_proc_start(player, buff_instance)
    return
end

function HudElementBuffBar:event_player_buff_proc_stop(player, buff_instance)
    return
end
...
function HudElementBuffBar:event_player_buff_removed(player, buff_instance)
    if not self._player or self._player ~= player then
        return
    end
...
function HudElementBuffBar:event_present_new_area(area_id)
...
function HudElementBuffBar:_add_buff(buff_instance)
    local active_buffs_data = self._active_buffs_data
...
function HudElementBuffBar:_remove_buff_at_index(index)
    local active_buffs_data = self._active_buffs_data
...
function HudElementBuffBar:_setup_buff_widget_array(ui_renderer)
...
function HudElementBuffBar:_get_available_widget = function (self)
...
function HudElementBuffBar:_return_widget(widget, ui_renderer)
    if not widget then
        return nil
    end
...
function HudElementBuffBar:_update_buff_alignments(force_update)
    local active_buffs_data = self._active_buffs_data
    local num_active_buffs = #active_buffs_data
    local horizontal_spacing = BuffBarSettings.horizontal_spacing
...
function HudElementBuffBar:_set_widget_state_colors(widget, is_negative, is_active)
    local source_colors = nil

    if not is_active then
        source_colors = BuffBarSettings.inactive_colors
    elseif is_negative then
        source_colors = BuffBarSettings.negative_colors
    else
        source_colors = BuffBarSettings.positive_colors
    end
...
function HudElementBuffBar:_destroy_widgets(ui_renderer)
...
function HudElementBuffBar:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
...
function HudElementBuffBar:_register_events = function (self)
    local event_manager = Managers.event
    local events = BuffBarSettings.events
...
function HudElementBuffBar:_unregister_events = function (self)
    local event_manager = Managers.event
    local events = BuffBarSettings.events
...
function HudElementBuffBar:destroy = function (self)
    ...
    HudElementBuffBar.super.destroy(self)
end

function HudElementBuffBar:set_visible(visible, ui_renderer, use_retained_mode)
...
function HudElementBuffBar:update(dt, t, ui_renderer, render_settings, input_service)
    HudElementBuffBar.super.update(self, dt, t, ui_renderer, render_settings, input_service)
